...
response = requests.get('https://github.com/GPflow/GPflow/blob/develop/doc/sphinx/notebooks/advanced/data/three_phase_oil_flow.npz?raw=true')
...
# %%
observed = mgpx.Dataset(y=observations)
objective = jit(gplvm.marginal_log_likelihood(observed, negative=True))

opt = ox.adam(0.05)
learned_params, history = gpx.abstractions.fit(objective, parameter_state, optax_optim=opt,n_iters=1000).unpack()
